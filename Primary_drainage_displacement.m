...
Lf = zeros(nt,1);
LfM = zeros(nt,1);  % 2022.11.14
Sw = zeros(nt,1);
Cwater = zeros(nt,1);
CwaterM = zeros(nt,1);  % 2022.11.14
Swc = zeros(nt,1);
k  = 1;
H  = zeros(nt,1);
Sarea = zeros(nt,1);
...
for f = 1:nt
    ...
    for j = 1:k:nt
        ...
        if check(j) == 1
            ...
            try
                [ocM(j), wcM(j), AoM(j), AwM(j)] = ...
                    drainconductanceforequilatral(pc, advang, geometry, ift, porearea(j), R(j), watervisco, oilvisco);  % 2022.11.14 
                CwaterM(j)  = AwM(j);
                LfM(j) = interfacialarea(geometry, bpd(j),ift, check(j), pc, oillayer(j), advang); % 2022.11.14 
            catch
                [ocM(j), wcM(j), AoM(j), AwM(j)] = ...
                    drainconductanceforequilatral(pc, advang(j), geometry, ift, porearea(j), R(j), watervisco, oilvisco);  % 2022.11.14
                CwaterM(j)  = AwM(j);
                LfM(j) = interfacialarea(geometry, bpd(j),ift, check(j), pc, oillayer(j), advang(j)); % 2022.11.14 
            end
