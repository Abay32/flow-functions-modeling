function [oc,wc,Ao,Aw] = Secondary_drainconductanceforequilatral(pc,recangle,geometry,ift,lswd, porearea, R, watervisco, oilvisco)
...
if geometry == 1
    ...
    elseif recangle > pi/2 - al
        r     = ift./pc;
        tetaH =   acos(lswd*sin(al)/r) - al;
        Aw    = cwater(r, tetaH);%Awc;%
        p3    = (pi/2 - al).*tan(al);
        num   = Aw.^2.*(1-sin(al)).^2.*tan(al).*p3.^2;
        denum = 12*nc*sin(al).^2.*(1-p3).*(1 + p3).^2;
        g     = num./denum;%corener water conductance 
        wc    = g/watervisco;
        %
        Ao    = porearea - Aw;
        go    = pi.*(sqrt(Ao/pi) + R).^4./128;
